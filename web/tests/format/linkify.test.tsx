...
describe("linkify", () => {
  it("leaves plain text unchanged", async () => {
    await render(<>{linkify("fix: update readme", { repo: "git-compose" })}</>);
    await expect.element(page.getByText("fix: update readme")).toBeVisible();
  });

  it("turns a bare URL into a link", async () => {
    await render(<>{linkify("see https://example.com/docs for details", { repo: "git-compose" })}</>);
  ...
  it("turns a bare commit sha into a repo-relative link", async () => {
    await render(<>{linkify("fixed by abc1234def", { repo: "git-compose" })}</>);
  ...
  it("does not link short hex runs below the 7-character minimum", async () => {
    await render(<>{linkify("see abc123 for context", { repo: "git-compose" })}</>);
