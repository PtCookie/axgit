...
Two adjacent defects noticed while implementing #81/#82, both dating to the original `feat(web):
initialize web project` scaffolding commit and never touched since:

- `web/public/favicon.svg` was the **stock Astro starter logo** (the Astro "A" glyph) — axgit never
  had a mark of its own to fall back to when `AXGIT_FAVICON` is unset, the common case.
- `web/public/favicon.ico` was a 32×32 **PNG with an `.ico` extension**, not an actual ICO
  container — browsers tolerate this, but it isn't what the filename claims.

Fixed alongside the logo/favicon feature rather than filed separately, since #81 already needed a
recognizable answer to "what does an unconfigured axgit look like" for its own default-icon-removal
tests. `favicon.svg` is now PtCookie's own git mark (a fixed dark-circle badge with a green
branch/node glyph, supplied directly rather than drawn for this commit — no `prefers-color-scheme`
swap, since the dark circle background is legible against both browser-chrome themes on its own);
`favicon.ico` is now a real 16×16 + 32×32 ICO container, generated by a small one-off script
(`web/scripts/make-favicon.mjs`, rasterizing the SVG via the Playwright already installed as a
`web/` devDependency and hand-writing the ICONDIR/ICONDIRENTRY header) rather than pulling in a new
dependency for a one-time asset build — the same "keep the generator script alongside its output"
