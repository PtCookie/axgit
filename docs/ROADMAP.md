...
- **Replace the scaffold favicon with an axgit mark** (docs/DECISIONS.md #84), the last piece of
  the logo/favicon feature (#81-#83). `web/public/favicon.svg` was still the unmodified Astro
  scaffold logo and `favicon.ico` was a PNG with an `.ico` extension, not a real ICO container —
  both untouched since the original scaffolding commit, and axgit never had a mark of its own to
  fall back to when `AXGIT_FAVICON` is unset (the common case). `favicon.svg` is now PtCookie's own
  git mark; `favicon.ico` is a real 16×16+32×32 ICO, generated by a new
  `web/scripts/make-favicon.mjs` (rasterizes via the Playwright already installed as a devDependency,
